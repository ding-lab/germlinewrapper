...
if (report_type == "N"){
  write.table(charged_cpgs, paste0("all.samples.NormalOnly.", batch_date, ".charged2vcf.filtered.RCfiltered.af0.0005.4manualreview.tsv"), quote = F, row.names = F, sep = "\t")
} else if (report_type == "TN"){
  write.table(charged_cpgs, paste0("all.samples.TN.", batch_date, ".charged2vcf.filtered.RCfiltered.af0.0005.4manualreview.tsv"), quote = F, row.names = F, sep = "\t")
} else {
  print("Invalid report type. Will not add it to filename")
  write.table(charged_cpgs, paste0("all.samples.", batch_date, ".charged2vcf.filtered.RCfiltered.af0.0005.4manualreview.tsv"), quote = F, row.names = F, sep = "\t")
}
